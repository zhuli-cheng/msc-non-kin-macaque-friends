...
all.predictors.analysis <- all.predictors %>%
  dplyr::select (focal.id,focal.behavioral.mother,group,year,per.kin.in.top3,top3.kin,per.nonkin.in.top3,top3.nonkin,mother.alive,mean.DSI.gregariousness,focal.connections,focal.kin.available,focal.age,focal.ordinal.rank,focal.percofsex.dominanted,focal.rank) %>%
  unique()
...
#linear model
predictors <- lmer(per.nonkin.in.top3 ~ mother.alive + mean.age + age.centerred + mean.rank + rank.centerred + mean.kina + kina.centerred + mean.connection + connections.centerred + (1|focal.id), data=f.predictors.analysis)
...
predictors.logit<-lmer(logit ~ mother.alive + mean.age + age.centerred + mean.rank + rank.centerred + mean.kina + kina.centerred + mean.connection + connections.centerred + (1|focal.id), data=f.predictors.analysis)
...
predictors.pb <- glmer(top3.nonkin/(top3.total)~mother.alive + mean.age + age.centerred + mean.rank + rank.centerred + mean.kina + kina.centerred + mean.connection + connections.centerred + (1|focal.id),weights=top3.total,family=binomial,data=f.predictors.analysis)
