---
title: "DSI grooming, F2010"
author: "Zhuli"
date: "23/06/2020"
output: html_document
---

```{r my-chunk, R.options = list(width = 1300)}
rm(list=ls())
library(dplyr)
library(reshape2)
...
f2010g<-cbind(f2010g,colsplit(f2010g$observation.session, "\\.", names = c("month", "day","year","focal","No."))) 

#calculate the duration of all interations of one dyad 
f2010g <- f2010g %>% 
  mutate_each(funs(toupper), "focal")  %>%
  group_by(groom.giver, groom.reciever,focal) %>% 
...
f2010g<-f2010g %>% 
  select (groom.giver,groom.reciever,duration,focal,hrs.focalfollowed,sex,age,percofsex.dominanted)
